...
export function BetaAttendance() {
  ...
  return <section className="space-y-4 text-slate-200">
    ...
    {!current ? <p role="status">Loading attendance…</p> : current.error ? <p role="alert">{current.error}</p> : <div className="overflow-x-auto"><table className="w-full text-left"><thead><tr>{['Property', 'Employee', 'Employee #', 'Timestamp (UTC)', 'Event'].map(h => <th className="p-3" key={h}>{h}</th>)}</tr></thead><tbody>
