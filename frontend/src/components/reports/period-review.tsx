...
export function PeriodReview() {
    ...
    async function run(fn: () => Promise<void>) { if (busy) return; const expected = scope; setBusy(true); setError(''); try {
        await fn();
    }
    catch (e: any) {
    ...
    {configure && <details><summary>Configure future approval steps</summary><p className="text-sm text-slate-400">This creates a one-step workflow for future submissions. Existing submissions retain their ordered workflow. The selected role must also have permission to approve hours for this branch.</p><select aria-label="Final approver role" className={field} value={approver} onChange={e => setApprover(e.target.value)}>{['ADMIN', 'OWNER', 'SUPER_ADMIN', 'MANAGER', 'LOCATION_ADMIN', 'SUPERVISOR'].map(r => <option key={r} value={r}>{displayLabel(r)}</option>)}</select><button type="button" aria-label="Save approval workflow" aria-busy={busy} className="admin-secondary-button rounded-lg border px-3 py-2 text-sm" disabled={busy} onClick={() => run(async () => { await periodApprovalApi.configure(selectedLocationId, [{ stepName: 'Final period review', approverRole: approver }]); })}>{busy ? 'Saving…' : 'Save approval workflow'}</button></details>}
    {review && <><div className="flex flex-wrap gap-4"><strong>Period: {displayLabel(review.period.status)}</strong><button type="button" aria-label="Refresh review" aria-busy={busy} className="admin-secondary-button rounded-lg border px-3 py-2 text-sm" disabled={busy} onClick={() => run(() => load(review.period.id))}>{busy ? 'Loading…' : 'Refresh review'}</button><button type="button" aria-label="Submit period" aria-busy={busy} className="admin-primary-button rounded-lg border px-4 py-2 text-sm font-semibold" disabled={busy || review.requireApproval === false || !canAct || !review.rows.length || review.orphanAttendance > 0 || review.rows.some((r: any) => r.incompleteShifts || r.pendingCorrections || ['SUBMITTED', 'IN_REVIEW', 'APPROVED'].includes(r.status))} onClick={() => run(async () => { await periodApprovalApi.submit(review.period.id, review.reviewToken); await load(review.period.id); })}>{busy ? 'Please wait…' : 'Submit period'}</button></div>
