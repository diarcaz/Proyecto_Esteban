...
export function ReportsView() {
  ...
  const handleDownloadPdf = async () => {
    await generateCorporatePayrollPdf(
      payrollData,
      activeLoc ? activeLoc.name : 'Sucursal Centro - MÉRIDA',
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-800 pb-4 print:hidden">
        <div>
          <h2 className="text-xl font-black text-white flex items-center gap-2">
            Nómina &amp; Reportes Corporativos LFT <span className="text-xs font-bold px-2.5 py-0.5 rounded-full bg-blue-500/20 text-blue-300 border border-blue-500/30">NexuStaff Payroll</span>
          </h2>
          <p className="text-xs text-slate-400 font-medium">
            Cálculo automático de recargos nocturnos, días festivos y horas extra dobles/triples según Ley Federal del Trabajo.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <FileText className="h-4 w-4" /> PDF Corporativo (con Logo &amp; QR)
          </button>
          <button
            onClick={handlePrint}
          ...
          >
            <Printer className="h-4 w-4" /> Vista Impresión
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-800 bg-slate-950/80 p-3.5 text-xs">
        <div className="flex items-center gap-2 text-slate-300 font-bold">
          ...
          <span>Reglas de Nómina Activas (LFT México):</span>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <span className="px-2.5 py-1 rounded-lg bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 text-[10px] font-mono font-bold flex items-center gap-1">
            <Moon className="h-3 w-3" /> Nocturno: +35%
          </span>
          <span className="px-2.5 py-1 rounded-lg bg-amber-500/10 text-amber-300 border border-amber-500/20 text-[10px] font-mono font-bold flex items-center gap-1">
            <Clock className="h-3 w-3" /> Dobles (&lt;=9h OT): 200%
          </span>
          <span className="px-2.5 py-1 rounded-lg bg-rose-500/10 text-rose-300 border border-rose-500/20 text-[10px] font-mono font-bold flex items-center gap-1">
            <Award className="h-3 w-3" /> Triples (&gt;9h OT): 300%
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 print:grid-cols-4">
        <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 space-y-1">
          <span className="text-[10px] text-slate-500 font-black uppercase">Sucursal Activa</span>
          <p className="text-sm font-black text-white">{activeLoc ? activeLoc.name : 'Todas las Sucursales'}</p>
        </div>
        <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 space-y-1">
          <span className="text-[10px] text-slate-500 font-black uppercase">Horas Totales Trabajadas</span>
          <p className="text-base font-black text-blue-400 font-mono">{formatHours(totalNetHours)}</p>
        </div>
        <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 space-y-1">
          <span className="text-[10px] text-slate-500 font-black uppercase">Desglose Horas Extra</span>
          <p className="text-xs font-extrabold text-amber-400 font-mono">
            Noc: {totalNightHours.toFixed(1)}h | Dob: {totalDoubleOtHours.toFixed(1)}h | Trip: {totalTripleOtHours.toFixed(1)}h
          </p>
        </div>
        <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 space-y-1 bg-gradient-to-br from-slate-900 to-emerald-950/40 border-emerald-500/30">
          <span className="text-[10px] text-emerald-400 font-black uppercase flex items-center gap-1">
            <DollarSign className="h-3 w-3" /> Costo Total Nómina (LFT)
          </span>
          <p className="text-lg font-black text-emerald-400 font-mono">${totalPayrollCost.toFixed(2)} MXN</p>
        </div>
      </div>
          ...
          <table className="w-full text-left text-xs text-slate-300">
            <thead className="bg-slate-950 text-[10px] font-black uppercase tracking-wider text-slate-400 border-b border-slate-800">
              <tr>
                ...
                <th className="px-4 py-3.5 text-right">TARIFA/H</th>
                <th className="px-4 py-3.5 text-right">H. REGULAR</th>
                <th className="px-4 py-3.5 text-right">H. NOCTURNA (+35%)</th>
                <th className="px-4 py-3.5 text-right">H. DOBLES (200%)</th>
                <th className="px-4 py-3.5 text-right">H. TRIPLES (300%)</th>
                <th className="px-4 py-3.5 text-right font-black text-emerald-400">TOTAL NÓMINA ($)</th>
              </tr>
            </thead>
