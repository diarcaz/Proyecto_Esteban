...
export default function SettingsPage() {
  ...
  return <section className="max-w-5xl space-y-6"><header><p className="eyebrow">Workspace administration</p><h2 className="page-title">Settings</h2><p className="text-sm text-slate-400">Manage branch configuration and terminal setup.</p></header><div className="grid md:grid-cols-2 gap-4">
    ...
    <article className="panel p-6 space-y-4"><ShieldCheck className="text-emerald-400"/><h3 className="font-semibold text-lg">Signed-in account</h3><p className="text-sm">{user?.name}</p><span className="status-badge">{user?.role}</span><p className="text-sm text-slate-400">Access is determined by your assigned permissions and branches.</p></article>
    <article className="panel p-6 space-y-4"><Settings className="text-slate-400"/><h3 className="font-semibold text-lg">Global policies & audit</h3><span className="status-badge">Not available in this beta</span><p className="text-sm text-slate-400">Policy editing and the audit viewer are not enabled here. No settings have been changed.</p></article>
  </div></section>;
}
