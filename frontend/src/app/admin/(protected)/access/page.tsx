...
export default function AccessUsers() {
    ...
    useEffect(() => { setCatalog(null); setRows([]); setForm(null); setReset(null); setPassword(''); setError(''); setNotice(''); setBusy(false); if (allowed)
    ...
    const cancel = () => { setForm(null); setReset(null); setPassword(''); setError(''); };
    async function submit(event: React.FormEvent) { event.preventDefault(); setBusy(true); setError(''); setNotice(''); const credential = password; setPassword(''); try {
        if (reset)
    ...
    function edit(row: any) { setReset(null); setPassword(''); setError(''); const grants = [...new Set<string>([...row.legacyPropertyIds, ...row.grants.map((g: any) => g.propertyId)])].map(id => ({ propertyId: id, permissions: [...new Set([...(row.globalPermissions || []), ...(row.grants.find((g: any) => g.propertyId === id)?.permissions || [])])] })); setForm({ ...row, grants }); }
    function toggleBranch(id: string) { setForm({ ...form, grants: form.grants.some((g: any) => g.propertyId === id) ? form.grants.filter((g: any) => g.propertyId !== id) : [...form.grants, { propertyId: id, permissions: [] }] }); }
    function togglePermission(id: string, permission: string) { setForm({ ...form, grants: form.grants.map((g: any) => g.propertyId !== id ? g : { ...g, permissions: g.permissions.includes(permission) ? g.permissions.filter((p: string) => p !== permission) : [...g.permissions, permission] }) }); }
 ...
 {form && <><div className="grid md:grid-cols-2 gap-4">{['firstName', 'lastName', 'email'].map(key => <label key={key}>{key === 'firstName' ? 'First name' : key === 'lastName' ? 'Last name' : 'Email'}<input aria-label={key} type={key === 'email' ? 'email' : 'text'} required maxLength={key === 'email' ? 254 : 100} disabled={key === 'email' && !!form.id} className={field} value={form[key]} onChange={e => setForm({ ...form, [key]: e.target.value })}/></label>)}<label>Role<select aria-label="Account role" className={field} value={form.role} onChange={e => setForm({ ...form, role: e.target.value })}>{catalog.roles.map((r: string) => <option key={r} value={r}>{displayLabel(r)}</option>)}</select></label><label>Company<select aria-label="Account company" required disabled={!!form.id} className={field} value={form.companyId} onChange={e => setForm({ ...form, companyId: e.target.value, grants: [] })}><option value="">Select company</option>{catalog.companies.map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label><label>Status<select aria-label="Account status" className={field} value={form.status} onChange={e => setForm({ ...form, status: e.target.value })}><option value="ACTIVE">Active</option><option value="TERMINATED">Inactive</option></select></label></div>
 {form.role === 'OWNER' && <p>An Owner has full access to branches within their company.</p>}
 <fieldset className="space-y-3"><legend className="font-semibold">Authorized branches and permissions</legend><p className="text-sm text-slate-400">Choose what this account can do at each branch. Viewing attendance includes time exports. Rate visibility requires separate permission.</p>{catalog.properties.filter((p: any) => p.companyId === form.companyId && (form.id ? p.canEdit : p.canCreate)).map((p: any) => { const grant = form.grants.find((g: any) => g.propertyId === p.id); return <div className="rounded-lg border border-slate-700 p-3" key={p.id}><label><input type="checkbox" checked={!!grant} onChange={() => toggleBranch(p.id)}/> {p.name}</label>{grant && <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2 mt-3">{p.permissions.map((permission: string) => <label className="text-xs" key={permission}><input type="checkbox" checked={grant.permissions.includes(permission)} onChange={() => togglePermission(p.id, permission)}/> {displayLabel(permission)}</label>)}</div>}</div>; })}</fieldset></>}
 {(reset || !form?.id) && <label className="block">Temporary password<input aria-label="Temporary password" type="password" autoComplete="new-password" required minLength={16} maxLength={72} className={field} value={password} onChange={e => setPassword(e.target.value)}/><span className="text-xs text-slate-400">16+ characters; upper/lowercase, number and symbol. Never displayed again after saving. No automatic first-login password change is enforced.</span></label>}
 {reset && <p className="text-sm text-slate-400">Resetting {reset.email}. Existing sessions are not automatically revoked. Deactivate the account to deny subsequent authenticated requests when necessary.</p>}
 <p className="text-sm text-slate-400">{reset ? 'Save to replace this account’s password.' : form?.status === 'TERMINATED' ? 'Saving will deactivate this account and block further access.' : 'Review the role and authorized branches before saving.'}</p><div className="flex gap-3"><button disabled={busy} className="rounded-lg bg-blue-600 p-3">{busy ? 'Saving…' : 'Save account'}</button><button type="button" disabled={busy} onClick={cancel}>Cancel</button></div></form>}
