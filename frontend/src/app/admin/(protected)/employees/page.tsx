...
export default function EmployeesPage() {
  ...
  async function action(work:()=>Promise<void>) { if(pending.current) return; pending.current=true; setBusy(true); setError(''); setNotice(''); try { await work(); } catch (e:any) { setError(staffError(e)); } finally { pending.current=false; setBusy(false); } }
  ...
  return <section className="space-y-5 text-slate-200">
    ...
    {mode && <form onSubmit={submit} className="space-y-4 rounded-xl border border-slate-700 bg-slate-900 p-5">
      ...
      {catalog && !catalog.departments.length && <p>No departments are configured for {catalog.propertyName || 'this Branch'}. {catalog.canManage ? 'Use Add Department below.' : 'Contact a Branch Administrator or Company Administrator with permission to configure this Branch.'}</p>}
      {catalog && assignment.departmentId && !catalog.positions.some((p:any)=>p.departmentId===assignment.departmentId) && <p>No positions are configured for this Department. {catalog.canManage ? 'Use Add Position below.' : 'Contact a Branch Administrator or Company Administrator with permission to configure this Branch.'}</p>}
      ...
      <div className="flex gap-3"><button disabled={busy||!canSubmit||!catalog||!assignment.positionId} className={buttonClass}>{busy?'Saving…':mode==='create'?'Save Staff Member':'Save Assignment'}</button><button type="button" disabled={busy} onClick={()=>{setMode(null);setIdentity(i=>({...i,pinCode:''}));}}>Cancel</button></div>
    </form>}
  </section>;
}
