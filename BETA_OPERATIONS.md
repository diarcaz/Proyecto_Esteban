...
Status: BETA FUNCTIONALLY READY for the local Time & Attendance journey. Phase 4.6 onboarding and browser acceptance passed. Remote deployment/proxy/backups remain operator gates. No remote service or customer data was created. Isolated local acceptance data was cleaned up.
...
4. Deploy the backend image with all variables present. Docker CMD is prisma migrate deploy && node dist/main.js. Migrations must complete successfully against the dedicated beta database before the app starts. No db push, reset, automatic seed or history edits. If deployment fails, diagnose before retrying; do not reset the database.
5. From an authenticated operator shell in the backend image/workdir run npx --no-install prisma migrate status. Confirm all six migration directories are applied, including 20260909000100_stage1_additive_reconciliation and 20260909000200_stage2_approval_step_fk. The legacy users.pin_code column/index remain deliberately untouched.
...
7. Explicitly run npm run bootstrap:admin from /app in the backend container (or an operator environment using the same image and dedicated beta connection). It hashes with bcrypt, creates the supplied company/admin in one transaction, serializes competing attempts and refuses existing admins/accounts or nonempty company/user tables. It does not reset an existing password and is never run by normal startup. If it refuses, inspect the existing state rather than delete data. Remove every BOOTSTRAP_* variable afterward.
